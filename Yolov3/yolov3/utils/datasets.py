...
from Yolov3.yolov3.utils.utils import xyxy2xywh, xywh2xyxy
...
class LoadImagesOnly:  # for inference
    def __init__(self, path, half=False):
        path = str(Path(path))  # os-agnostic
        files = []
        if os.path.isdir(path):
            files = sorted(glob.glob(os.path.join(path, '*.*')))
    ...
    def __next__(self):
        ...
        #print('img0:',img0.shape) #(240, 360, 3)
        #print('img:',img.shape) #(3, 224, 320)
...
class LoadTensor:
    def __init__(self, tensor, half=False, channel_in=1):
        ...
        self.mode = 'images'
        self.half = half  # half precision fp16 images
        assert self.nF > 0, 'No images found in the input tensor'
    ...
    def __next__(self):
        ...
        frame_tensor = self.tensor[:, self.count, :, :]

        # 将 tensor 转换为 NumPy 格式 (im0)
        # 1. 转到 CPU, 分离计算图, 转 NumPy
        im0 = frame_tensor.cpu().detach().numpy()
        # 2. 从 [-1, 1] 转到 [0, 255]
        im0 = (im0 + 1) * 127.5
        # 3. 转换数据类型
        im0 = im0.astype(np.uint8)
        # 4. 将 [C, H, W] 转为 [H, W, C] (OpenCV 格式)
        im0 = np.transpose(im0, (1, 2, 0))

        # !! 如果是 1 通道, im0 shape 是 [H, W, 1], 需要转为 [H, W, 3] 灰度图
        if self.channel_in == 1:
            im0 = cv2.cvtColor(im0, cv2.COLOR_GRAY2RGB)  # [H, W, 1] -> [H, W, 3]
        # (如果是 3 通道, im0 shape 是 [H, W, 3] 彩色, 无需操作)
        # !! ----------------------------------- !!

        # Padded resize (letterbox 现在处理的是正确的彩色或灰度图)
        img = letterbox(im0)[0]

        # Convert for model input (YOLO 通常需要 RGB)
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB (如果 im0 是灰度, 这里不变)
        img = np.ascontiguousarray(img, dtype=np.float16 if self.half else np.float32)
        img /= 255.0  # 0 - 255 to 0.0 - 1.0

        self.count += 1
        # 返回模型输入 img (总是 3 通道) 和 原始帧 im0 (根据 channel_in 可能是彩色或灰度)
        return img, im0
...
def letterbox(img, new_shape=[224,320], color=(128, 128, 128),
              auto=True, scaleFill=True, scaleup=False, interp=cv2.INTER_AREA):
